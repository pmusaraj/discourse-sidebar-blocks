...
export default createWidget("sidebar-category-posts", {
  ...
  buildClasses(attrs) {
    const result = [];
    if (attrs.category) {
      result.push(`sidebar-c-${attrs.category}`);
    }
    if (attrs.tag) {
      result.push(`sidebar-tag-${attrs.tag}`);
    }
    return result;
  },
  ...
  refreshTopics(attrs, state) {
    ...
    state.loading = true;
    attrs.count =
      attrs.count || parseInt(this.siteSettings.sidebar_num_results);
  ...
  html(attrs, state) {
    ...
    var heading = "";
    if (attrs.category) {
      var category = Category.findBySlug(attrs.category);
